...
//!
//! Exercises the real HTTP endpoint and SQLite refresh-token store.
...
/// The `jti` of a refresh token is its primary key in `refresh_token`.
...
/// Read the expiry from the same SQLite database the test PDS uses.
fn stored_expiry(dir: &Path, token_id: &str) -> String {
    let conn = rusqlite::Connection::open(dir.join("account.sqlite")).unwrap();
    conn.query_row(
        "SELECT \"expiresAt\" FROM refresh_token WHERE id = ?1",
        [token_id],
        |row| row.get(0),
    )
...
async fn refresh_session_sets_a_two_hour_reuse_grace_window() {
    ...
    let grace_expiry = from_str_to_utc(&stored_expiry(dir.path(), &old_jti)).unwrap();
...
async fn create_session_stores_a_ninety_day_refresh_token_expiry() {
    ...
    let expiry = from_str_to_utc(&stored_expiry(dir.path(), &jti_of(&refresh_jwt))).unwrap();
