...
fn fixture() -> &'static Fixture {
    &FIXTURE
        .get_or_init(|| {
            ...
            // The reference fixture served its own OAuth authorization server.
            // Keep that choice scoped to clients booted from this fixture.
            let fixture_hostname = secrets["PDS_HOSTNAME"].as_str().unwrap();
            std::env::set_var(
                "PDS_OAUTH_AUTHORIZATION_SERVER",
                format!("https://{fixture_hostname}"),
            );
            std::env::set_var(
                "PDS_ADMIN_PASS",
