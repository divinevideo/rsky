...
async fn oauth_well_known_documents() {
    ...
    // The fork advertises its external entryway by default, while keeping
    // the built-in authorization server's own metadata and keys available.
    assert_eq!(
        body["authorization_servers"],
        json!([rsky_pds::config::DEFAULT_ENTRYWAY_URL])
    );
