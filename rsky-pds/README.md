...
This version stores PDS state in SQLite and does not read the previous
PostgreSQL database. Its schema migrator initializes and upgrades SQLite
databases; it does not convert a PostgreSQL installation. Building this
version successfully is therefore only the source integration step.

Before switching an existing deployment, validate a separate data migration
in staging, including account and session state, repository blocks and
indexes, sequencer state, signing keys, and access to the existing blobstore.
Preserve the previous database and service configuration for recovery. See
[MIGRATING-1.0.md](MIGRATING-1.0.md) for the separate per-account signing-key
migration; rotating keys is not a substitute for migrating stored data.
