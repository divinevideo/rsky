...
impl<Base: AccessTier> Scoped<RpcProxy, Base> {
    /// The subject of a proxy request whose method and audience were checked
    /// by the request guard. `RpcProxy` has no deferred handler-side check.
    pub fn requester_did(&self) -> Option<String> {
        self.access
            .credentials
            .as_ref()
