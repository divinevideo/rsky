...
const REFUSE_BOUNDARY_FN: &str = "CREATE OR REPLACE FUNCTION wintermute_test_refuse_boundary() RETURNS trigger \
    LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'injected write failure'; END $$;";
const REFUSE_ACK_FN: &str = "CREATE OR REPLACE FUNCTION wintermute_test_refuse_ack_write() RETURNS trigger \
    LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'injected write failure'; END $$;";
...
async fn a_failed_write_after_the_boundary_keeps_the_fence_for_the_workflow() {
    ...
    client
        .batch_execute(&format!(
            "{REFUSE_BOUNDARY_FN} DROP TRIGGER IF EXISTS wintermute_test_refuse_boundary_record ON record; \
             CREATE TRIGGER wintermute_test_refuse_boundary_record BEFORE INSERT ON record FOR EACH ROW \
             WHEN (NEW.did = '{did}') EXECUTE FUNCTION wintermute_test_refuse_boundary();"
        ))
        .await
        .unwrap();
    let err = reconcile(&pool, &pds.client, &options(did))
    ...
    client
        .batch_execute("DROP TRIGGER wintermute_test_refuse_boundary_record ON record")
        .await
        .unwrap();
    let err = reconcile(
        &pool,
...
async fn a_commit_acknowledgement_is_retried_until_it_is_durable() {
    ...
    client
        .batch_execute(&format!(
            "{REFUSE_ACK_FN} DROP TRIGGER IF EXISTS wintermute_test_refuse_ack ON wintermute.did_progress; \
             CREATE TRIGGER wintermute_test_refuse_ack BEFORE INSERT OR UPDATE ON wintermute.did_progress \
             FOR EACH ROW WHEN (NEW.did = '{did}') EXECUTE FUNCTION wintermute_test_refuse_ack_write();"
        ))
        .await
        .unwrap();
    let commit = IndexJob {
    ...
    client
        .batch_execute(&format!(
            "DROP TRIGGER IF EXISTS wintermute_test_refuse_record ON record; \
             ...
             WHEN (NEW.did = '{did}') EXECUTE FUNCTION wintermute_test_refuse_ack_write();"
        ))
        .await
        .unwrap();
    let record = job(
